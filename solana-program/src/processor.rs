...
use crate::{
    error::ZynthexError,
    instruction::ZynthexInstruction,
    state::*,
};
...
pub fn process_instruction(
    ...
    let instruction = ZynthexInstruction::try_from_slice(instruction_data)
        .map_err(|_| ZynthexError::InvalidInstruction)?;

    match instruction {
        ZynthexInstruction::InitializeOrganization { org_slug } => {
            process_initialize_organization(program_id, accounts, org_slug)
        }
        ZynthexInstruction::InitializeWorkspace { workspace_id, name } => {
            process_initialize_workspace(program_id, accounts, workspace_id, name)
        }
        ZynthexInstruction::CommitVersion { content_hash, message } => {
            process_commit_version(program_id, accounts, content_hash, message)
        }
        ZynthexInstruction::CreateFork { fork_workspace_id, fork_at_version, name } => {
            process_create_fork(program_id, accounts, fork_workspace_id, fork_at_version, name)
        }
        ZynthexInstruction::CreatePullRequest { title, source_version_hash, target_version_hash } => {
            process_create_pull_request(program_id, accounts, title, source_version_hash, target_version_hash)
        }
        ZynthexInstruction::ApprovePullRequest => {
            process_approve_pull_request(program_id, accounts)
        }
        ZynthexInstruction::MergePullRequest { merge_commit_hash, message } => {
            process_merge_pull_request(program_id, accounts, merge_commit_hash, message)
        }
        ZynthexInstruction::RecordExecution { version_hash, result_hash } => {
...
fn process_create_fork(
    ...
    if fork_at_version > parent_workspace.current_version {
        return Err(ZynthexError::InvalidVersion.into());
    }
...
fn process_approve_pull_request(
    ...
    if !pr.is_initialized {
        return Err(ZynthexError::InvalidPRState.into());
    }
...
fn process_merge_pull_request(
    ...
    if pr.status != PRStatus::Approved {
        return Err(ZynthexError::PRNotApproved.into());
    }
