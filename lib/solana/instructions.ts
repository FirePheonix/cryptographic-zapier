...
import { zynthex_PROGRAM_ID, zynthexInstruction } from "./config";
...
export function derivePDA(
  ...
  programId: PublicKey = zynthex_PROGRAM_ID
): [PublicKey, number] {
...
export function createInitializeOrganizationInstruction(
  ...
  const data = Buffer.from([zynthexInstruction.InitializeOrganization]);
...
export function createWorkspaceInstruction(
  ...
  const data = Buffer.from([zynthexInstruction.CreateWorkspace]);
...
export function createCommitVersionInstruction(
  ...
  const instructionData = Buffer.concat([
    Buffer.from([zynthexInstruction.CommitVersion]),
...
export function createForkInstruction(
  ...
  const data = Buffer.from([zynthexInstruction.CreateFork]);
...
export function createPullRequestInstruction(
  ...
  const instructionData = Buffer.concat([
    Buffer.from([zynthexInstruction.CreatePullRequest]),
...
export function createMergePullRequestInstruction(
  ...
  const data = Buffer.from([zynthexInstruction.MergePullRequest]);
...
export function createRecordExecutionInstruction(
  ...
  const instructionData = Buffer.concat([
    Buffer.from([zynthexInstruction.RecordExecution]),
    Buffer.from(versionHash.padEnd(32, "\0")),
